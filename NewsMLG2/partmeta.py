...
from .contentmeta import AdministrativeMetadataGroup, DescriptiveMetadataGroup, Icon
...
class TimeDelim(CommonPowerAttributes):
    ...
    """
    attributes = {
        # The start time of the part in a timeline. The expressed time unit is excluded. Using the Edit Unit requires the frame rate or sampling rate to be known, this must be defined by the referenced rendition of the content.</xs:documentation>
        'start': 'start',  # type="xs:string" use="required">
        # The end time of the part in a timeline. The expressed time unit is included. Using the Edit Unit requires the frame rate or sampling rate to be known, this must be defined by the referenced rendition of the content.</xs:documentation>
        'end': 'end',  # type="xs:string" use="required">
        # The unit used for the start and end timestamps - expressed by a QCode / either the timeunit or the timeunituri attribute MUST be used</xs:documentation>
        'timeunit': 'timeunit',  # type="QCodeType">
        # The unit used for the start and end timestamps - expressed by a URI / either the timeunit or the timeunituri attribute MUST be used</xs:documentation>
        'timeunituri': 'timeunituri',  # type="IRIType">
        # Refers to the content rendition with this QCode as rendition attribute value - expressed by a QCode</xs:documentation>
        'renditionref': 'renditionref',  # type="QCodeType">
        # Refers to the content rendition with this QCode as rendition attribute value - expressed by a URI</xs:documentation>
...
class RegionDelim(CommonPowerAttributes):
    ...
    """
    attributes = {
        # The x-axis coordinate of the side of the rectangle which has the smaller x-axis coordinate value in the current user coordinate system</xs:documentation>
        'x': 'x',  # type="xs:integer">
        # The y-axis coordinate of the side of the rectangle which has the smaller y-axis coordinate value in the current user coordinate system</xs:documentation>
        'y': 'y',  # type="xs:integer">
...
class PartMetaPropType(AdministrativeMetadataGroup, DescriptiveMetadataGroup, I18NAttributes):
    """
    A type representing the structure of a partMeta property
    """
    elements = {
        'icon': { 'type': 'array', 'xml_name': 'icon', 'element_class': Icon },
        'timedelim': { 'type': 'array', 'xml_name': 'timeDelim', 'element_class': TimeDelim },
        'regiondelim': { 'type': 'single', 'xml_name': 'regionDelim', 'element_class': RegionDelim },
        'role': { 'type': 'single', 'xml_name': 'regionDelim', 'element_class': RegionDelim },
        'partmetaextproperty': { 'type': 'array', 'xml_name': 'partMetaExtProperty', 'element_class': PartMetaExtProperty },
